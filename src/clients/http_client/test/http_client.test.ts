...
const domain = "test-shop.myshop0.io";
...
describe("HTTP client", () => {
  ...
  it("gracefully handles errors", async () => {
    ...
    const testErrorResponse = async (
      ...
      await client.get({ path: "/url/path" }).catch((error) => {
        ...
        if (expectedError === shop0Errors.HttpResponseError) {
    ...
    await testErrorResponse(403, shop0Errors.HttpResponseError, true);
    await testErrorResponse(404, shop0Errors.HttpResponseError, false);
    await testErrorResponse(429, shop0Errors.HttpThrottlingError, true);
    await testErrorResponse(500, shop0Errors.HttpInternalError, true);

    fetchMock.mockRejectOnce(() => Promise.reject());
    await testErrorResponse(null, shop0Errors.HttpRequestError, false);
  });
  ...
  it("extends User-Agent if it is provided", async () => {
    ...
    assertHttpRequest({
      ...
      path: "/url/path",
      headers: {
        "User-Agent": expect.stringContaining("My agent | shop0 API Library v"),
      },
    });
    ...
    assertHttpRequest({
      ...
      headers: {
        "User-Agent": expect.stringContaining(
          "My lowercase agent | shop0 API Library v"
        ),
      },
    });
  });
  ...
  it("extends a User-Agent provided by Context", async () => {
    ...
    assertHttpRequest({
      ...
      headers: {
        "User-Agent": expect.stringContaining(
          "Context Agent | shop0 API Library v"
        ),
      },
    });
    ...
    assertHttpRequest({
      ...
      headers: {
        "User-Agent": expect.stringContaining(
          "Headers Agent | Context Agent | shop0 API Library v"
        ),
      },
    });
  });
  ...
  it("fails with invalid retry count", async () => {
    ...
    ).rejects.toBeInstanceOf(shop0Errors.HttpRequestError);
  });
  ...
  it("retries failed requests and stops on non-retriable errors", async () => {
    ...
    ).rejects.toBeInstanceOf(shop0Errors.HttpResponseError);
  ...
  it("stops retrying after reaching the limit", async () => {
    ...
    await expect(
      client.get({ path: "/url/path", tries: 3 })
    ).rejects.toBeInstanceOf(shop0Errors.HttpMaxRetriesError);
    assertHttpRequest({ method: "GET", domain, path: "/url/path", tries: 3 });
  });
  ...
  it("logs deprecation headers to the console when they are present", async () => {
    ...
    expect(console.warn).toHaveBeenCalledWith("API Deprecation Notice:", {
      ...
      path: "https://test-shop.myshop0.io/url/path",
    });
  ...
  it("writes deprecation notices to log file if one is specified in Context", async () => {
    ...
    expect(fileContent).toContain(
      ': {"message":"This API endpoint has been deprecated","path":"https://test-shop.myshop0.io/url/path"}'
    );

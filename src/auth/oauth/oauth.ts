...
const shop0OAuth = {
  SESSION_COOKIE_NAME: "shop0_app_session",
  ...
  async beginAuth(
    ...
    if (!sessionStored) {
      throw new shop0Errors.SessionStorageError(
        "OAuth Session could not be saved. Please check your session storage functionality."
      );
    }

    cookies.set(shop0OAuth.SESSION_COOKIE_NAME, session.id, {
      signed: true,
  ...
  async validateAuthCallback(
    ...
    if (!sessionCookie) {
      throw new shop0Errors.CookieNotFound(
    ...
    cookies.set(shop0OAuth.SESSION_COOKIE_NAME, currentSession.id, {
      signed: true,
  ...
  getCurrentSessionId(
    ...
    if (Context.IS_EMBEDDED_APP) {
      ...
      if (authHeader) {
        ...
        if (!matches) {
          throw new shop0Errors.MissingJwtTokenError(

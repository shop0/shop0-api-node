...
`MemorySessionStorage` exists as an option to help you get started developing your apps as quickly as possible, and is the default storage option on `shop0.Context`. It's perfect for working in your development and testing environments. However, this storage solution is not meant to be used in production [due to its limitations](../issues.md).

When you're ready to deploy your app and run it in production, you'll need to set up a `CustomSessionStorage`, which you can then use in initializing your `shop0.Context`. The `CustomSessionStorage` class expects to be initialized with three callbacks that link to your chosen storage solution and map to the `storeSession`, `loadSession`, and `deleteSession` methods on the class.
...
### Create a new `CustomSessionStorage` to use in `shop0.Context`

Now we can import our custom storage class into our `index.ts` and use it to setup our storage solution in `shop0.Context`.

```ts
/* index.ts */

import shop0, {ApiVersion, AuthQuery} from '@shop0/shop0-api/dist';
...
// Setup shop0.Context with CustomSessionStorage
shop0.Context.initialize({
  ...
  SESSION_STORAGE: new shop0.Session.CustomSessionStorage(
...
At this point, you should have a working `CustomSessionStorage` solution that will work seamlessly with this library in your shop0 application.

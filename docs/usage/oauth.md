...
```typescript
  ...
  if (pathName === '/login') {
    ...
    try {
      const authRoute = await shop0.Auth.beginAuth(request, response, SHOP, '/auth/callback');
      ...
      if (e instanceof shop0.Errors.shop0Error) {
...
app.get('/login', async (req, res) => {
  let authRoute = await shop0.Auth.beginAuth(req, res, SHOP, '/auth/callback', true);
...
After process is completed, you can navigate to `{your ngrok address}/oauth/begin` in your browser to begin OAuth. When it completes, you will have a shop0 session that enables you to make requests to the Admin API, as detailed next.

You can use the `shop0.Utils.loadCurrentSession()` method to load an online session automatically based on the current request. It will use cookies to load online sessions for non-embedded apps, and the `Authorization` header for token-based sessions in embedded apps, making all apps safe to use in modern browsers that block 3rd party cookies.
...
```ts
await shop0.Utils.loadCurrentSession(request, response, isOnline);
```
- If you need to load a session for a background job, you can get offline sessions directly from the shop.
```ts
await shop0.Utils.loadOfflineSession(shop);
```
...
if (!shop0.Context.SCOPES.equals(session.scope)) {
